...
namespace PixelCrushers.DialogueSystem.Wrappers
{
    ...
    public class StandardDialogueUI : PixelCrushers.DialogueSystem.StandardDialogueUI
    {
        ...
        private Subtitle _previousSubtitle; // kostyl
        ...
        private void AddPhraseToHistory(Subtitle subtitle)
        {
            ...
            {
                _scrollRect.verticalNormalizedPosition = Mathf.Abs(delta - _scrollBar.size);
            }
        ...
        private void ScrollToBottom()
        {
            ...
            scrollCoroutine = StartCoroutine(ScrollToBottomCoroutine(_scrollRect));
        }

        IEnumerator ScrollToBottomCoroutine(ScrollRect sr)
        {
            while (sr.verticalNormalizedPosition > 0f)
            {
                sr.verticalNormalizedPosition -= Time.deltaTime * scrollSpeed;

                yield return null;
            }

            delta = _scrollBar.size;
